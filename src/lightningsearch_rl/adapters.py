...
def convert_2wiki_file(raw_path: Path, corpus_path: Path, examples_path: Path) -> None:
    rows = json.loads(raw_path.read_text(encoding="utf-8"))
    passages: list[Passage] = []
    examples: list[dict] = []
    for row in rows:
        row_passages = _passages_from_2wiki_context(row["context"])
        passages.extend(row_passages)
        corpus_doc_ids = [passage.doc_id for passage in row_passages]
        gold_doc_ids = [
...
def _passages_from_2wiki_context(context: dict[str, list[str]]) -> list[Passage]:
    passages: list[Passage] = []
    for title, sentences in context.items():
        for sentence_index, sentence in enumerate(sentences):
            passages.append(
                Passage(
                    doc_id=_doc_id("2wiki", title, sentence_index),
                    title=title,
                    text=sentence,
                )
            )
